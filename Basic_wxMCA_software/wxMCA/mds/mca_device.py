...
BOOT_LOADER = 0x1000 # MCA boot loader

pid_list = [EMORPHO_PID, PMT1K_MCA_PID, SIPM1K_MCA_PID, PMT2K_MCA_PID, SIPM2K_MCA_PID, PMT3K_MCA_PID, SIPM3K_MCA_PID, 
            PMT_COUNTER_PID, SIPM_COUNTER_PID, PMT_N3K_PID, SIPM_N3K_PID, COUNTER_3K, HV_ARM, BOOT_LOADER]  # Valid product ID's
    ...
    def find_all(self):
        ...
        while bool(bus):
            ...
            while bool(dev):
                ...
                pid = dev[0].descriptor.idProduct
                if pid == 0x1000:
                    print("Found MCA in boot loader mode. PID: {:X}, VID: {:X}".format(pid, vid))
                else:
    ...
    def make_mca_dict(self):
        ...
        for handle, pid, dev in zip(self.handles, self.pids, self.devices):
            ...
            if pid == 0x6001:  # eMorpho
                ...
                #ret0 = libusb0.usb_bulk_read(handle, self.cmd_in_ep, buffer_out_p, 64, self.usb_read_timeout)

                #print("Read returned {} bytes.".format(ret1), ret0, count)
                #print(bytes_out)
                #print(bytes_in)
