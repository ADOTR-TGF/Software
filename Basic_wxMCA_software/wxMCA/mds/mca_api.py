...
item_size_dict = {'B': 1, 'H': 2, 'h': 2, 'I': 4, 'i': 4, 'f': 4}  # Bytes per data item, depending on the data_type
...
class arm_api():
    def __init__(self):
        ...
        """
        self.make_new_io_counter = {  # Counter-1000
        ...
        self.make_new_io_counter3k = {  # Counter-3000
        ...
        self.make_new_io_obj_1k = {  # MCA-1000
            ...
            'arm_cmd': mca1k_data.arm_cmd
        }
        self.make_new_io_obj_2k = {  # MCA-2000
            'arm_ping': mca2k_data.arm_ping,
            'arm_version': mca2k_data.arm_version,
            'arm_status': mca2k_data.arm_status,
            'arm_ctrl': mca2k_data.arm_ctrl,
            'arm_cal': mca2k_data.arm_cal,
            'arm_histogram': mca2k_data.arm_histogram,
            'arm_histo_4k': mca2k_data.arm_histogram_4k,
            'arm_bck': mca2k_data.arm_bck,
            'arm_diff': mca2k_data.arm_diff,
            'arm_logger': mca2k_data.arm_logger,
            'arm_trace': mca2k_data.arm_trace,
            'arm_weights': mca2k_data.arm_weights,
            'arm_listmode': mca2k_data.arm_listmode,
            'arm_cmd': mca2k_data.arm_cmd,
            'arm_spi_fpga': mca2k_data.arm_spi_fpga,
            'arm_spi_mem': mca2k_data.arm_spi_mem
        }
        self.make_new_io_obj_3k = {  # MCA-3000
            ...
            'fpga_lm_nrl1': mca3k_data.fpga_lm_nrl1,
            'fpga_tgf_ctrl': mca3k_data.fpga_tgf_ctrl,
            'fpga_tgf_results': mca3k_data.fpga_tgf_results,
            'fpga_tgf_lm': mca3k_data.fpga_tgf_lm,
            'fpga_tgf_xt28k': mca3k_data.fpga_tgf_xt28k,
            'fpga_tgf_xt2k_1': mca3k_data.fpga_tgf_xt2k_1,
            'fpga_tgf_xt2k_2': mca3k_data.fpga_tgf_xt2k_2,
            'fpga_tgf_xt2k_3': mca3k_data.fpga_tgf_xt2k_3,
            'fpga_tgf_xt2k_4': mca3k_data.fpga_tgf_xt2k_4,
            'arm_version': mca3k_data.arm_version,
            'arm_status': mca3k_data.arm_status,
            'arm_ctrl': mca3k_data.arm_ctrl,
            'arm_cal': mca3k_data.arm_cal,
            'arm_cmd': mca3k_data.arm_cmd
        }
        ...
        self.make_new_io_obj_hvarm = {  # ARM controlled high voltage generator
            'arm_ping': hvarm_data.arm_ping,
            'arm_version': hvarm_data.arm_version,
            'arm_status': hvarm_data.arm_status,
            'arm_ctrl': hvarm_data.arm_ctrl,
            'arm_logger': hvarm_data.arm_logger
        }
        self.make_new_io_obj_bootloader = {  # Boot loader controls
            'arm_ping': bootloader_data.arm_ping,
            'arm_version': bootloader_data.arm_version,
            'arm_status': bootloader_data.arm_status,
            'arm_cmd': bootloader_data.arm_cmd,
            'arm_boot': bootloader_data.arm_boot,
            'arm_rom': bootloader_data.arm_rom
        }
    ...
    def perform_cmd(self, mca):
        ...
        io_obj_dict = {
            0x005: self.make_new_io_counter3k,
            0x100: self.make_new_io_counter,
            0x200: self.make_new_io_counter,
            0x101: self.make_new_io_obj_1k,
            0x201: self.make_new_io_obj_1k,
            0x102: self.make_new_io_obj_2k,
            0x202: self.make_new_io_obj_2k,
            0x103: self.make_new_io_obj_3k,
            0x203: self.make_new_io_obj_3k,
            0x104: self.make_new_io_obj_n3k,
            0x204: self.make_new_io_obj_n3k,           
            0x106: self.make_new_io_obj_hvarm,
            0x1000: self.make_new_io_obj_bootloader
        }
        io_obj = io_obj_dict[mca.mca_id][mca.cmd["name"]]()
        ...
        call = {  # Dictionary of functions to call
    ...
    def short_write(self, mca, io_obj):
        ...
        if io_obj.data_type in ['H', 'I', 'f']:
            struct.pack_into("<{}{}".format(mca.cmd["num_items"], io_obj.data_type), mca.bytes_out, 4, *mca.cmd_out_list[0:num_items])
    ...
    def data_write(self, mca, io_obj):
        ...
        mca.num_bytes = num_items*item_size_dict[io_obj.data_type]
        mca.bytes_out = array.array('B', [0] * max(64, mca.num_bytes))  # Create a send byte buffer

        try:
            struct.pack_into("<{}{}".format(num_items, io_obj.data_type), mca.bytes_out, 0, *mca.data_out_list[0:num_items])
        except:
            print(num_items, mca.data_out_list)
            print(mca.num_bytes)
...
class ftdi_api():
    ...
    def __init__(self):
        ...
        self.make_new_io_obj = {
            ...
            'fpga_lut': emorpho_data.fpga_lut,
            'fpga_weights': emorpho_data.fpga_weights,
            'fpga_time_slice': emorpho_data.fpga_time_slice,
            'fpga_lm_2b': emorpho_data.fpga_lm_2b,
            'fpga_lm_nrl1': emorpho_data.fpga_lm_nrl1,
            'fpga_tgf_ctrl': emorpho_data.fpga_tgf_ctrl,
            'fpga_tgf_results': emorpho_data.fpga_tgf_results,
            'fpga_tgf_lm': emorpho_data.fpga_tgf_lm,
            'fpga_tgf_xt28k': emorpho_data.fpga_tgf_xt28k,
            'fpga_tgf_xt2k_1': emorpho_data.fpga_tgf_xt2k_1,
            'fpga_tgf_xt2k_2': emorpho_data.fpga_tgf_xt2k_2,
            'fpga_tgf_xt2k_3': emorpho_data.fpga_tgf_xt2k_3,
            'fpga_tgf_xt2k_4': emorpho_data.fpga_tgf_xt2k_4,
        }
    ...
    def perform_cmd(self, mca):
        ...
        if mca.cmd['memory'] == "lut" and mca.cmd['dir'] == "read":
            self.read_lut(mca)
            return None
        ...
        if mca.cmd['memory'] == "lut" and mca.cmd['dir'] == "write":
            self.write_lut(mca)
            return None
    ...
    def read_ram(self, mca, io_obj):
        ...
        if 'num_items' not in mca.cmd or mca.cmd['num_items'] == 0:
            mca.cmd['num_items'] = io_obj.num_items
        off = int(offset*item_size_dict[io_obj.data_type]/512)
        ...
        mca.read_data(item_size_dict[io_obj.data_type])  # Read the data, fills mca.bytes_in
        # mca.data_in_list = struct.unpack_from("<{}{}".format(mca.cmd['num_items'], io_obj.data_type), mca.bytes_in)
        if io_obj.data_type == "f":  # convert the 4-byte pattern from integers to floats
            L = len(mca.data_in_list)
            iar = array.array("I", mca.data_in_list)
            mca.data_in_list = struct.unpack_from("<{}f".format(L), iar)
        io_obj.registers = mca.data_in_list
        io_obj.registers_2_fields()
        io_obj.fields_2_user()
    ...
    def read_flash(self, mca):
        ...
        mca.data_out_dict["fields"] = {"read_nv": 1, "sel_lut": 0}
        ...
        #print("nv_mem_valid", nv_mem_valid)
        if nv_mem_valid:
    ...
    def write_flash(self, mca):
        ...
        ctrl_obj.fields.update({"write_nv": 1, "sel_lut": 0})
    ...
    def read_lut(self, mca):
        """ Read 256 bytes from the LUT area in the configuration flash
        """
        # Copy nv-mem content into user area
        ctrl_obj = self.make_new_io_obj["fpga_ctrl"]()
        ctrl_obj.adc_sr = mca.adc_sr
        mca.data_out_dict["fields"] = {"read_nv": 1, "sel_lut": 1}
        self.read_modify_write_ram(mca, ctrl_obj)
        time.sleep(0.02)

        # read data from the user area
        mca.cmd.update({"num_items": 64})
        user_obj = self.make_new_io_obj["fpga_lut"]()
        self.read_ram(mca, user_obj)

        # Report LUT data
        nv_mem_valid = -60 < mca.data_in_list[0] < 100
        print("Found valid LUT:", nv_mem_valid)
        if nv_mem_valid:
            mca.data_in_dict = {"registers": user_obj.registers, "fields": user_obj.fields}
        else:
            mca.data_in_dict = {"registers": user_obj.registers}

        return None


    def write_lut(self, mca):
        # read fpga_ctrl
        ctrl_obj = self.make_new_io_obj["fpga_ctrl"]()
        ctrl_obj.adc_sr = mca.adc_sr
        self.read_ram(mca, ctrl_obj)

        # Write LUT to user area
        # If the user gives "fields" in the command, they must describe the entire LUT.
        # This is not a read-modify-write command.
        user_obj = self.make_new_io_obj["fpga_lut"]()
        if "fields" in mca.data_out_dict:  # If "fields" are present, they overwrite the registers
            user_obj.fields = mca.data_out_dict["fields"]
            user_obj.fields_2_registers()
            mca.data_out_list = user_obj.registers

        # Write to eMorpho user area
        fmt = user_obj.data_type
        L = item_size_dict[fmt]  # Number of bytes per item in list (4 bytes for a float)
        num_items = 32//L
        num_chunks = len(mca.data_out_list) // num_items  # 32-byte data blocks
        mca.num_bytes = 34  # Each outgoing packet is 32 bytes plus a 2-byte header
        
        mca.cmd_out_list = [0.0] * num_items  # data values for each packet
        mca.bytes_out = array.array('B', [0] * mca.num_bytes)
        for n in range(num_chunks):
            # Make the packet header for the first write buffer
            ph = user_obj.cmd_addr << 8
            if n == 0:
                ph += 0x1
            struct.pack_into("<1H", mca.bytes_out, 0, ph)    
            mca.cmd_out_list = mca.data_out_list[n*num_items: (n + 1)*num_items]
            struct.pack_into("<{}{}".format(num_items, fmt), mca.bytes_out, 2, *mca.cmd_out_list)
            mca.write_data()  # Write to the data_out endpoint

        # Write fpa_ctrl to initiate transfer to SPI flash memory.
        ctrl_obj.fields.update({"write_nv": 1, "sel_lut": 1})
        ctrl_obj.fields_2_registers()
        mca.data_out_list = ctrl_obj.registers
        self.write_ram(mca, ctrl_obj)
        time.sleep(0.06)
        
        # The transfer completes in 50ms, typically.  If necessary,
        # check fpga_results["status"] & 0x80
    ...
    def read_modify_write_ram(self, mca, io_obj):
        ...
        # Update the dictionary with the user-supplied data and convert back to an updated list
        # Automatically set the "program_hv" bit for backwards compatibility
        update_hv = False
        if "fields" in mca.data_out_dict:
            if "dac_data" in mca.data_out_dict["fields"]:
                update_hv = True
            io_obj.fields.update(mca.data_out_dict["fields"])
            io_obj.fields_2_user()
        if "user" in mca.data_out_dict:
            if "high_voltage" in mca.data_out_dict["user"]:
                update_hv = True
            io_obj.user.update(mca.data_out_dict["user"])
            io_obj.user_2_fields()
