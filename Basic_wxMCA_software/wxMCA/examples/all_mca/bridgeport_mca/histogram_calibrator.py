...
class HistogramCalibrator():
    ...
    def save_histogram(self, out_file=None, comment="", mode=""):
        ...
        elif self.MCA.mca_id in  [ self.SIPM1K, self.PMT1K, self.SIPM2K, self.PMT2K]:
        ...
        op_volt_dg = self.get_op()
        out_str = '{{"S/N": {}, "T": {:.2f}, "LED": {:.2f}, "HV": {:.2f}, "DG": {:.2f}, "amplitude": {}, "peak": {:.3f} }}\n'.format(self.sn, temp_led["temperature"], temp_led["led_value"], op_volt_dg["ov"], op_volt_dg["digital_gain"], op_volt_dg["ha_mode"], self.peak_pos)
        with open("../data/{}_short_log.json".format(self.sn),"a") as fout:
            fout.write(out_str)
    ...
    def save_pulses(self, num_pulses, out_file=None, comment=""):
        ...
        """
        if self.MCA.mca_id in [ self.SIPM1K, self.PMT1K]:  # These have no pulse capture capability
            return

        if self.MCA.mca_id == self.EMORPHO:
            io_dict = {"fpga_results": "fpga_results", "pulse": "pulse"}

        elif self.MCA.mca_id in  [ self.SIPM2K, self.PMT2K, self.SIPM3K, self.PMT3K]:
    ...
    def refresh(self):
        ...
        if self.mca_id in [self.EMORPHO, self.PMT2K, self.SIPM2K, self.PMT3K, self.SIPM3K]:
            self.histo = self.histogram["registers"]
        elif self.mca_id in [self.PMT1K, self.SIPM1K]:
    ...
    def cal_mca(self):
        ...
        self.peak_pos = peak_pos # For recording
        kev_select = {self.EMORPHO: "emorpho_keV_bin", 
                      self.PMT1K:   "pmt1k_keV_bin",
                      self.PMT2K:   "pmt2k_keV_bin",
                      self.PMT3K:   "pmt3k_keV_bin",
                      self.SIPM1K:  "sipm1k_keV_bin",
                      self.SIPM2K:  "sipm2k_keV_bin",
                      self.SIPM3K:  "sipm3k_keV_bin"
                     }[self.mca_id]
        kev_bin = float(self.op_ctrl[kev_select])
        ...
        if self.mca_id in [self.EMORPHO, self.PMT2K, self.SIPM2K, self.PMT3K, self.SIPM3K] and ha_mode:
        ...
        if self.mca_id in [self.EMORPHO, self.PMT2K, self.SIPM2K, self.PMT3K, self.SIPM3K] and ha_mode or \
           self.mca_id in [self.PMT1K, self.SIPM1K]: # Adjust operating voltage
            new_ov = self.compute_new_ov(cal_ov, cal_ratio, inv_gain_exp)

        if self.mca_id in [self.EMORPHO, self.PMT2K, self.SIPM2K, self.PMT3K, self.SIPM3K] and not ha_mode: # We adjust the digital gain
    ...
    def get_temp_led(self):
        ...
        """
        par = {}
        par["led_value"] = 0  # Keep at zero for those devices that have no LED
        if self.mca_id in [self.EMORPHO]:
            res = self.MCA_IO.submit_command(self.sn, self.commands["fpga_results"])[self.sn]
            par["temperature"] = res["user"]["temperature"]
        if self.mca_id in [self.EMORPHO, self.PMT3K]:
            par["led_value"] = res["fields"]["rr_10"]

        if self.mca_id in [self.PMT1K, self.SIPM1K, self.PMT2K, self.SIPM2K, self.PMT3K, self.SIPM3K]:
            res = self.MCA_IO.submit_command(self.sn, self.commands["arm_status"])[self.sn]
            par["temperature"] = res["fields"]["avg_temperature"]
        if self.mca_id in [self.PMT1K, self.PMT2K]:
    ...
    def get_op(self):
        ...
        if self.mca_id in [self.PMT2K, self.SIPM2K]:
            par["ha_mode"] = arm_ctrl["user"]["amplitude"]
    ...
    def set_op(self, ov, dg): # Reprogram operating voltage and digital gain
        ...
        # Program a new digital gain into the device
        if self.mca_id in [self.PMT2K, self.SIPM2K]:
            self.MCA_IO.submit_command(self.sn, self.commands["write_arm_ctrl"], {"fields": {"cal_dg": dg}})
    ...
    def compute_new_ov(self, ov_old, ratio=1.0, exp=1.0/5.6):  # ratio>1 => ov_new > ov_old
        if self.mca_id in [self.EMORPHO, self.PMT1K, self.PMT2K, self.PMT3K]:  # PMT-based
            ov_new = ov_old * ratio**(exp/2)  # Divisor 2 is a relaxation parameter to improve stability.
        else:  # SiPM-based
            Vbr = {self.SIPM1K: 30.0, self.SIPM2K: 28.6, self.SIPM3K: 28.6}[self.mca_id]
            dov = (Vbr - ov_old)*(1-ratio)
            ov_new = ov_old + dov*0.125  # dov is multiplied by a relaxation parameter to improve stability.
    ...
    def calibrator_loop(self):
        ...
        if self.mca_id in [self.PMT2K, self.SIPM2K]:
            data_amplitude = {"user": {"amplitude": 1}}
            data_energy = {"user": {"amplitude": 0}}
        if self.mca_id in [self.PMT3K, self.SIPM3K]:
