...
class MCA_PORTAL():
    ...
    def __init__(self, cmd_root="", ctrl_root="", mds_ip=""):
        ...
        with open("{}requested_data.json".format(ctrl_root), 'r') as fin:
            self.requested_data = json.loads(fin.read())
        ...
        for detail in self.details:  # Collect MCA id's and ADC speed
            sn = list(detail)[0]
            self.sn_list += [sn]
            mca_id_str = detail[sn]["mca_id_str"] 
            self.mca[sn] = MCA(detail)
            self.mca[sn].mca_type = self.mca_controls["device_types"][mca_id_str]
            cmd_file = "{}{}".format(cmd_root, self.mca_controls["command_files"][mca_id_str])
            with open(cmd_file, "r") as fin:
                self.mca[sn].commands = json.loads(fin.read())
    ...
    def requested_sn_list(self, lun_sn):
        ...
        for item in lun_sn:
            if type(item)==str:
                self.req_list += [item]
            elif type(item)==int and item < len(self.sn_list):
                self.req_list += [self.sn_list[item]]
                
    def save_data(self, par):
        MCA = par["mca"]
        data_type = par["data_type"]
        if data_type not in self.requested_data:
            print("mca_portal.py: save_requested_data: The data_type {} is not supported.".format(data_type))
            return
        mode_list = self.requested_data[data_type]["mode_list"]
        par["requested_data"] = self.requested_data[data_type]["requested_data"]
        #print(par["requested_data"])
        for sn in self.req_list:
            # In requested_data we name all the quantities to be saved
            if par["mode"] not in mode_list:
                print("{} is not in {} => Selected {}.".format(par["mode"], mode_list, mode_list[0]))
                par["mode"] = mode_list[0]

            io_dict = par["requested_data"][MCA.mca_type][par["mode"]]
            out_dict = {"comment": par["comment"], "serial_number": sn, "SN": MCA.SN, "short_sn": MCA.short_sn,
                        "mca_id_str": MCA.mca_id_str}
            for key in io_dict:  # Read all data to be saved and assemble the output dictionary
                ret = self.submit_command(sn, MCA.commands[io_dict[key]])[sn]
                ret_dict = {}
                for item in par["items"]:  # select the items to save (registers, fields, user)
                    ret_dict[item] = ret[item]
                if key == "histogram" and MCA.mca_type=="emorpho":
...
class MCA():
    def __init__(self, detail):
        self.sn = list(detail)[0]
        self.mca_id_str = detail[self.sn]["mca_id_str"]
        self.mca_id = detail[self.sn]["mca_id"]
        self.short_sn = detail[self.sn]["short_sn"]
        self.adc_sr = detail[self.sn]["adc_sr"]
        self.SN = self.sn[0:min(8, len(self.sn))]
        self.has_fpga = self.mca_id in [0x6001, 0x103, 0x203, 0x104, 0x204]
        self.has_arm = self.mca_id in [0x100, 0x200, 0x101, 0x201, 0x102, 0x202, 0x103, 0x203, 0x104, 0x204]
        
        self.cmd = {"type": "mca_cmd", "dir": "read", "memory": "ram"}  #default cmd
