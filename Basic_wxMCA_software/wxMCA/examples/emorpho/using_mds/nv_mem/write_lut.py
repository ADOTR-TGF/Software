...
def write_lut(lut_file):
    ...
    cmd = {"type": "mca_cmd", "name": "fpga_lut", "dir": "write", "memory": "lut", "sn": sn,
           "data": { "fields": lut}}
    msg = mds_client.send_and_receive(json.dumps(cmd).encode('utf-8')).decode('utf-8')
    ret = json.loads(msg)
    ...
    msg = mds_client.send_and_receive(json.dumps(cmd).encode('utf-8')).decode('utf-8')
    ret = json.loads(msg)
    with open("./data/returned_lut.json", "w") as fout:
        fout.write(json.dumps(ret, indent=4))

lut_file = "./data/lut_eRC4660.json"
write_lut(lut_file)
