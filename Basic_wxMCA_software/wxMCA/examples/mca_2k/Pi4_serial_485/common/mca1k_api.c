...
void mca_write(struct mca_command *mca_cmd){
    ...
    switch(mca_cmd->com_address){
        case ARM_CTRL:
        case ARM_CAL:
            printf("writing buffer with write_type\n");
            bpi_write_buffer(mca_cmd -> cmd, 64);
            timer_wait(5555); // time to wait for writing 64 bytes data at 115200Hz baud rate, in micro seconds. 
            printf("start writing f_data into buffer\n");
            bpi_write_buffer((uint32_t *)(mca_cmd -> f_data), mca_cmd->num_bytes);
            timer_wait(22222); // time to wait for writing 256 bytes data at 115200Hz baud rate, in micro seconds. 
            printf("writing buffer done\n");
            break;
        default:
            break;
    }
...
void mca_read(struct mca_command *mca_cmd){
    ...
    switch(mca_cmd->com_address){
        ...
        case ARM_CTRL:
        case ARM_CAL:
            printf("writing buffer with read_type\n");
            bpi_write_buffer(mca_cmd -> cmd, 64);
            timer_wait(5555); // time to wait for writing 64 bytes data at 115200Hz baud rate, in micro seconds. 
            printf("start reading f_data from buffer\n");
