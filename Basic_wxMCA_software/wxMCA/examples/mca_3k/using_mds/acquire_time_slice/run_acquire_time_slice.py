...
def acquire_time_slice(num_slices, file_name):
    ...
    cmd = {"type": "mca_cmd", "name": "fpga_action", "dir": "rmw", "sn": sn,
           "data": {"fields": {"clear_statistics": 1, "clear_histogram": 1}}}
    ...
    for slice_number in range(num_slices):
        ...
        while True:  # Poll until one or more time slices are available
            ...
            num_buffers = ret[sn]["user"]["num_buffers"]  # Number of 105ms time slices that are available
            #print("{:X}, {:d}".format(ret[sn]["fields"]["status"], ret[sn]["fields"]["status"]//0x400))
            #time.sleep(0.05)
            if num_buffers > 0:
                break
        # Read slice data
...
acquire_time_slice(num_slices=100, file_name="./data/time_slices.dat")
