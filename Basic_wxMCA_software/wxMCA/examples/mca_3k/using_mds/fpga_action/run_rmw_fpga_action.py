...
def read_fpga_ctrl(file_name):
    ...
    # Read modify and write FPGA action registers
    cmd = {"type": "mca_cmd", "name": "fpga_action",  "dir": "rmw", "sn": sn,
       "data": {"fields": {"clear_led": 1, "clear_statistics": 1, "clear_histogram": 1}, "user":{}}}
    mds_client.send_and_receive(json.dumps(cmd).encode('utf-8')).decode('utf-8')

    # Read actions and store (Note AR0 and AR2 are self-clearing registers and always read as zero.)
    cmd = {"type": "mca_cmd", "name": "fpga_action", "dir": "read", "sn": sn}
    msg = mds_client.send_and_receive(json.dumps(cmd).encode('utf-8')).decode('utf-8')
    fpga_ctrl = json.loads(msg)[sn]
