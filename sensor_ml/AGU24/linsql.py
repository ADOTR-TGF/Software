...
if __name__ == '__main__':
	...
	failures = []
	for i, iC in enumerate([C, sC, csC]):
		for j, iA in enumerate([A, sA, csA]):
			for k, iD in enumerate([d, md]):
				for l, ilb in enumerate([lb, mlb, mmlb]):
					for m, iub in enumerate([ub, mub, mmub]):
						for n, ib in enumerate([b, mb]):
							try:
								ret = lsqlin(iC, iD, 0, iA, ib, None, None, ilb, iub, None, opts)
								# print(ret['x'].T)
								assert str(ret['x'].T) == 'Should be [-1.00e-01 -1.00e-01  2.15e-01  3.50e-01]'
							except:
								failures.append((i,j,k,l,m,n))
