...
def wiener_deconvolve(s, kernel, noise_stdev):
    # note this handles gaussian noise, not quantization error/noise

    signal_fft = np.fft.fft(s)
    kernel_fft = np.fft.fft(kernel, n=len(s))
    r = signal_fft / kernel_fft

    # https://www.cs.sfu.ca/~yagiz/cpim/2019-CPIM-12-Deconvolution.pdf
    snr = signal_fft / noise_stdev
    factor = np.abs(kernel_fft) / (np.abs(kernel_fft) + 1 / snr)
    r *= factor
