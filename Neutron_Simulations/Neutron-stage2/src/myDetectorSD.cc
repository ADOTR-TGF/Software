...
G4bool myDetectorSD::ProcessHits(G4Step* step, G4TouchableHistory*)
{
	...
	// check if particle is just entering detection volume and kill the track 
	// and secondaries 	

  G4StepPoint* point1 = step->GetPreStepPoint();
  if (point1->GetStepStatus() != fGeomBoundary) return false;
...
void myDetectorSD::EndOfEvent(G4HCofThisEvent*)
{
  ...
  if (fcount_events >= SIZE_DATA_BUFFER){
    for (G4int k=0; k < fcount_events; k++){
      fout_events << setw(12) << fevtNb[k]
                  << setw(18) << fEdepEvt[k]
                  << endl;
