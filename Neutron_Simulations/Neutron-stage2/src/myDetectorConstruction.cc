...
G4VPhysicalVolume* myDetectorConstruction::Construct()
{
  ...
  // Aircraft Layer
  //
    int X = 50; // 0 is ground level, 1 is 100m, 2 is 200m, 10 is 1km, 30 is 3km etc.. 
	// TUBS
	G4double aircraft_pRmin = 0.0;
	G4double aircraft_pRmax = pRmax;
	G4double aircraft_pDz =   0.5*m; // half-length
	G4double aircraft_pSPhi = 0.0; 
	G4double aircraft_pDPhi = 2.0*pi;

	G4double aircraft_z = -(fslabs_pDz[X] - aircraft_pDz); 
	

	// Material
	char aircraftmaterialName[50];
	sprintf(aircraftmaterialName, "SlabMat%02i", X);
  G4Material* aircraft_mat = nist->FindOrBuildMaterial(aircraftmaterialName);
	//G4cout << aircraft_mat << G4endl;

  // position and orientation
  G4ThreeVector aircraft_position = G4ThreeVector(0,0,aircraft_z);
  G4Transform3D aircraft_transform = G4Transform3D(yRot90deg, aircraft_position);

  G4Tubs* solidAIRCRAFT
    = new G4Tubs("AIRCRAFT",              //its name
                aircraft_pRmin,           //its size 
                aircraft_pRmax,
                aircraft_pDz,
                aircraft_pSPhi,
                aircraft_pDPhi);
  ...
  fLogicSD = 
    new G4LogicalVolume(solidAIRCRAFT,    //its solid
                        aircraft_mat,     //its material
                        "AIRCRAFT");      //its name

  new G4PVPlacement(aircraft_transform, 
            				fLogicSD,  			    //its logical volume
            				"AIRCRAFT",     			  //its name
            				(fLogicSlabSDs+X),  //its mother (logical) volume
            				false,        			//no boolean operations
            				0,            			//its copy number 
            				false);        			//checkOverlap
...
void myDetectorConstruction::ConstructSDandField()
{
	...
	G4VSensitiveDetector *AIRCRAFTSD;
	char SDName[128];
	sprintf(SDName, "AIRCRAFT_%s", fParticleInputPrefix->c_str());
	
	AIRCRAFTSD = new myDetectorSD(SDName, "AIRCRAFTHitsCollection");
	SDman->AddNewDetector(AIRCRAFTSD);
	fLogicSD->SetSensitiveDetector(AIRCRAFTSD);
