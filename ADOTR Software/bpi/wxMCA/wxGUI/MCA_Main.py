...
class MCA_Frame(wx.Frame):
    ...
    def __init__(self):
        ...
        print(snnames[1]+','+ snnames[3])
        snnum = int(input('Enter a number from 0 to 1 if 2 SNs show:'))
        ...
        if self.MCA["mca_id"] in  [ 0x6001, 0x203, 0x103]:
            self.list_mode_wx = list_mode_wx.ListModeWindow(None,  'List Mode')
        ...
        if menu_item in self.display_function[self.mca_id]:
            if menu_item == 'list_mode_wx':
                try:
                    ...
                    self.display_function[self.mca_id][menu_item]()
            else:
                self.display_function[self.mca_id][menu_item]()
