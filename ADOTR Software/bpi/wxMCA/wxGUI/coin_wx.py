...
class HistogramWindow(wx.Frame): 
    def __init__(self, parent, title): 
        ...
        self.disp_ctrl = self.MCA["display_controls"]["coincidence"]["data"]
        self.display_name = "coincidence"  # The name of the entry in the display_controls.json file; used by graph_ctrl_grid
        ...
        # Add a droptown selection box for coincidence
        self.choice_box = wx.BoxSizer(wx.HORIZONTAL)
        choices = ["0", "1", "2", "3"]
        self.coin_text = wx.StaticText(self, wx.ID_ANY, "  Coincidence:  ")
        self.coin_text.SetBackgroundColour(self.bck_color)
        self.coin_choice_box = wx.Choice(panel, choices = choices)
        self.coin_choice_box.SetSelection(0)
        self.coin_selected = 0
        self.choice_box.Add(self.coin_text, 0, wx.RIGHT, 20)
        self.choice_box.Add(self.coin_choice_box, 0, wx.RIGHT, 102)
        self.box_graph.Add(self.choice_box, 0, wx.BOTTOM| wx.TOP, 0)
        button = wx.Button(panel, id = wx.ID_ANY, label="Save as", name="save_histogram") 
        button.Bind(wx.EVT_BUTTON, self.OnClicked)
        self.choice_box.Add(button, 0, wx.RIGHT, 5)
        ...
        self.choice_box.Add(self.save_choice_box, 0, wx.RIGHT, 4)
        self.coin_choice_box.Bind(wx.EVT_CHOICE, self.OnCoinChoice)
        self.save_choice_box.Bind(wx.EVT_CHOICE, self.OnSaveChoice)
        # Make a list of action buttons
        self.button_list = []
        button = wx.Button(panel, id = wx.ID_ANY, label="New", name="start_coinc")
    ...
    def OnCoinChoice(self, event):
        self.coin_selected = self.coin_choice_box.GetSelection()
    ...
    def OnClicked(self, event): 
        ...
        if name in ["refresh", "fit", "smooth", "save_histogram"]:
    ...
    def save_histogram(self):
        ...
        short_sn = '_' + out_dict['arm_version']['user']['unique_sn'][:8] + '_coin_' + str(self.coin_selected) + '.'
        ...
        if self.save_selected == 0: 
            ...
            with wx.FileDialog(self, "Save histogram data as xml", wildcard="xml files (*.xml)|*.xml", defaultDir = data_path, defaultFile = file_name_record.replace(".json", "_N42.xml"),
                            style=wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT) as fileDialog:
    ...
    def refresh(self):
        ...
        cmd_histo["ctrl"] = [self.coin_selected]
        ...
        if self.mca_id in [0x6001, 0x103, 0x203]:
            ...
            # Extract rate data from the individual coincidence histogram
            run_time = self.histogram["user"]["run_time"]
            count_rate = self.histogram["user"]["count_rate"]
            count_rate_err = 0
            #print(self.histogram["user"], self.histogram["registers"][0:75])
...
def main():             
    app = wx.App() 
    panel = HistogramWindow(None,  'Histogram')
    panel.Show()
    app.MainLoop()
