...
class HistogramWindow(wx.Frame): 
            ...
            with wx.FileDialog(self, "Save histogram data as csv", wildcard="csv files (*.csv)|*.csv", defaultDir = data_path, defaultFile = file_name_record.replace(".json", ".csv"),
                ...
                try:
                    ...
                    with rates_csv_file:
                        ...
                        keys = ["arm_version", "arm_status", "arm_ctrl", "histo"]
                        for key in keys:
                            for k in out_dict[key]["fields"].keys():
                                if k == "histogram":
                                    continue
                                rates_csv_writer.writerow([k, out_dict[key]["fields"][k]])
    ...
    def cal_mca(self):
        ...
        if self.mca_id in [self.EMORPHO, self.PMT3K, self.SIPM3K] and not ha_run:  # We have adjusted the digital gain
