...
void bpi_write_buffer(uint32_t *pSrc, uint32_t num_bytes){
    ...
    uint8_t ping_send = 0xA5;  // A ping byte to send
    uint8_t ping_recv = 1;     // Store the received ping byte
    ...
    if(frac_length>0){
        ...
        bpi_serial_read((uint8_t *)&ping_recv, 1);       // Blocking read of 1 byte
        bpi_serial_reset_buffers();                      // Empty buffers
        bpi_serial_write((uint8_t *)pSrc, frac_length);  // Send data
    }
}
...
void bpi_read_buffer(uint32_t *pDst, uint32_t num_bytes){
    const uint32_t chunk_size = 256;
    uint8_t ping_send = 0xFF;  // A ping byte to send
    uint8_t ping_recv = 1;  // Store the received ping byte
    uint32_t num_chunks, frac_length;
