...
def log_voltage(file_name):
    ...
    dwell_time = 1  # in seconds
    num_data = 20*3600 # Number of voltage readings to record.
    
    #mds_ip = "tcp://192.168.1.18:5507"
    mds_ip = "tcp://127.0.0.1:5507"
    mds_client = com.zmq_device(mds_ip, "client")
    ...
    for n in range(num_data):
        ...
        ret = json.loads(msg)
        arm_status = ret[sn]
        #data_out = {"dt": dwell_time, "op_voltage": arm_status["fields"]["op_voltage"]}
        time.sleep(dwell_time)
    
        with open(file_name, 'a') as fout:
            fout.write(json.dumps(arm_status["registers"]) + "\n")
