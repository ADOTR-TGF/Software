...
void myDetectorSD::EndOfEvent(G4HCofThisEvent*)
{
  ...
  G4double EdepEvt = 0;
  //G4double EdepTrack = 0;

	//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@

	/*
	// iterate over hits within given trackID
	G4double currTrackID; 
	G4double prevTrackID = 1;
	for ( G4int i=0; i<nofHits; i++){
    myDetectorHit *aHit = (*fHitsCollection)[i]; 
			
		currTrackID = aHit->GetTrackID();	

	}	
	*/
	//std::sort(fHitsCollection->GetVector()->begin(), fHitsCollection->GetVector()->end());

	//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@

	std::vector<myDetectorHit*> *myHits(fHitsCollection->GetVector());
	std::sort(myHits->begin(), myHits->end(),  &myDetectorHit_less);

	// iterate over hits in event
  for ( G4int i=0; i<nofHits; i++ ){
    //myDetectorHit *aHit = (*fHitsCollection)[i]; 
		myDetectorHit *aHit = (*myHits)[i];
