...
myPrimaryGeneratorAction::myPrimaryGeneratorAction(double medianBinEnergy) //string particleInputFilename
: G4VUserPrimaryGeneratorAction(), fParticleGun(0), fEnvelopeOrb(0)
{
	...
	string displayE = std::to_string(medianBinEnergy);
	cout << "Simulation energy: " << displayE << G4endl;
...
void myPrimaryGeneratorAction::GeneratePrimaries(G4Event *anEvent)
{
  ...
  fParticleGun->SetParticlePosition(G4ThreeVector(StartXYZ[0]*R, StartXYZ[1]*R, StartXYZ[2]*R + .25*m));
